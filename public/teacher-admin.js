...
(function () {
  ...
  function panel(){
    ...
    return '<section class="pcard mint noprint teacher-control-center" id="teacher-admin-panel"><div class="teacher-control-head"><div><span class="teacher-eyebrow">TEACHER CONTROL</span><h2>🧑‍🏫 เมนูครู</h2><p>เลือกงานที่ต้องการได้ทันที · มีนักเรียนในฐานข้อมูล '+count+' บัญชี</p></div>'+button('↻ โหลดข้อมูลล่าสุด','refreshTeacherSupabaseDashboard()','whiteb')+'</div>'+
      ...
      '<div class="teacher-tool-row teacher-password-reset"><label><span>ตั้งรหัสผ่านใหม่ให้นักเรียน</span><div class="password-wrap"><input id="admin-new-password" class="inp" type="password" minlength="4" maxlength="20" autocomplete="new-password" placeholder="รหัสใหม่ 4–20 ตัว"><button type="button" class="password-eye" aria-label="ดูรหัสผ่าน" aria-pressed="false" onclick="togglePasswordVisibility(\'admin-new-password\',this)">👁️</button></div></label>'+button('🔑 บันทึกรหัสใหม่','teacherSetStudentPassword()','blueb')+'</div>'+
      '<div class="innerbox" style="padding:14px;margin-top:14px"><h4>♻️ รีเซ็ตความคืบหน้า</h4><p class="soft small">คงชื่อเล่น บัญชี และผลก่อนเรียนไว้ ล้างบทเรียน คะแนน ดาว เหรียญ และตู้สุ่ม พร้อมเติมสิทธิ์สุ่มใหม่ 5 ครั้ง</p><div style="display:flex;gap:8px;flex-wrap:wrap">'+button('รีเซ็ตบัญชีที่เลือก',"teacherAdminReset('all',false)",'pinkb')+button('รีเซ็ตนักเรียนทั้งห้อง',"teacherAdminReset('all',true)",'pinkb')+'</div></div></div><div id="teacher-admin-editor"></div></section>';
  }
  ...
  window.teacherContentEditorHtml=function(){
    return '<section class="pcard blue noprint" id="teacher-content-menu" style="margin-bottom:20px"><h3>แก้ไขเนื้อหา</h3><div class="innerbox" style="padding:12px;margin:12px 0"><p data-teacher-save-status role="status">'+esc(draftNotice())+'</p>'+button('💾 บันทึกทุกเมนู','teacherAdminSaveAll()','mintb')+'</div><div class="innerbox" style="padding:14px;margin:12px 0"><b>📚 แก้ไขแยกตามบท</b><p class="soft small">เลือกบทเพื่อแก้ชื่อ เนื้อหา สื่อ สถานการณ์ กิจกรรม เกณฑ์ผ่าน และข้อสอบทุกข้อ</p><div style="display:flex;gap:8px;flex-wrap:wrap">'+lessons.map(function(l,i){return button('บทที่ '+(i+1),'teacherAdminLesson('+i+')','blueb');}).join('')+'</div></div><div style="display:flex;gap:8px;flex-wrap:wrap">'+Object.keys(moduleNames).filter(function(k){return k!=='lessons';}).map(function(k){return button(moduleNames[k],"teacherAdminContent('"+k+"')");}).join('')+button('บทเรียนทั้งหมด',"teacherAdminContent('lessons')")+'</div></section>';
  };
  ...
  function field(value,path,label){
    ...
    if(key==='answer'){var parent=get(path.slice(0,-1));if(parent&&Array.isArray(parent.choices))return '<label style="display:block;margin:9px 0">'+esc(label)+'<select class="inp" style="width:100%" onchange="'+change+'">'+parent.choices.map(function(choice,i){return '<option value="'+i+'" '+(Number(value)===i?'selected':'')+'>'+(['ก','ข','ค','ง','จ','ฉ','ช','ซ'][i]||String(i+1))+'. '+esc(choice)+'</option>';}).join('')+'</select></label>';}
