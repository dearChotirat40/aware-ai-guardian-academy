...
-- Then set the private PIN from the SQL Editor with:
-- select public.configure_teacher_pin('your-private-pin-at-least-8-characters');
create extension if not exists pgcrypto;

create or replace function public.configure_teacher_pin(p_pin text)
returns void language plpgsql security definer set search_path = public, extensions
as $$
begin
  if length(coalesce(p_pin, '')) < 8 then
    raise exception 'Teacher PIN must contain at least 8 characters';
  end if;
  insert into public.app_settings(id, data, updated_at)
  values (
    'teacher_security',
    jsonb_build_object('pin_hash', extensions.crypt(p_pin, extensions.gen_salt('bf', 10))),
