...
(async () => {
  ...
  await db.exec(`create table app_curriculum(id text primary key,data jsonb); alter table cabinet_wallets add column bonus_tickets integer not null default 0;`);
  await db.exec(readFileSync('supabase-lesson-draw-tickets.sql','utf8'));
  ...
  await db.query("update students set data = data || $1::jsonb where id='bob'",[JSON.stringify({lp:[{quizDone:true},{quizDone:true},{quizDone:false},{quizAttempted:true},{}]})]);
  assert.equal((await call('bob')).tickets,2);
  assert.equal((await call('bob')).tickets,2); // Reloading/reviewing does not grant more.
  const lessonDraw=randomUUID();
  assert.equal((await call('bob',lessonDraw)).tickets,1);
  assert.equal((await call('bob',lessonDraw)).tickets,1);
  await db.query("update students set data = data || $1::jsonb where id='bob'",[JSON.stringify({lp:Array.from({length:5},()=>({quizDone:true}))})]);
  assert.equal((await call('bob')).tickets,4); // Five lessons + one original badge - two draws.
